...
import { PriceCacheItem, JobStatusItem, JobStatus, ProcessingSettings, NormalizedRow } from './types';  // Fixed: local import
...
interface ProcessedRow extends NormalizedRow {
...
interface ISBNdbBook {
  ...
  subjects?: string[];
  msrp?: string;       // only on paid ISBNdb tiers
  msrp_txt?: string;
  price?: string;
  binding?: string;
}
...
interface ISBNdbBulkResponse {
  data: ISBNdbBook[];  // ISBNdb returns "data", not "books"
  total: number;
  requested: number;
}
...
async function processFile(bucket: string, key: string, jobId: string) {
  ...
  // Download and parse uploaded document
  const fileData = await downloadFile(bucket, key);
  const fileName = jobStatus.fileName || key;
  const rows = await parseUploadedDocument(fileData, fileName);
...
async function buildPriceMap(isbns: string[], priceMap: Map<string, number>, apiKey: string, date: string) {
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    try {
      ...
      console.log(`Processed batch ${i + 1}, found ${(booksData.data?.length ?? 0)} books, priceMap size: ${priceMap.size}`);
...
async function fetchBulkDataFromISBNdb(isbns: string[], apiKey: string): Promise<ISBNdbBulkResponse> {
  try {
    // ISBNdb bulk endpoint requires POST with JSON body, not GET with URL path params
    ...
    if (!response.ok) {
      ...
      if (response.status === 404) {
        ...
        return { data: [], total: 0, requested: 0 };
      }
      
      throw new Error(`ISBNdb API error: ${response.status} ${response.statusText}`);
    }

   const data = (await response.json()) as ISBNdbBulkResponse;
    return data;
...
function extractPriceFromBook(book: ISBNdbBook): number | null {
  // ISBNdb list price is available on all plans but not every book record has it populated.
  // msrp and msrp_txt are the standard fields; price is included as a safety net.
  const candidates = [book.msrp, book.msrp_txt, book.price];
  for (const raw of candidates) {
    if (raw) {
      const priceStr = raw.toString().replace(/[^0-9.]/g, '');
  ...
  // No price data found for this book in ISBNdb — row will be marked no_price_found
  return null;
}

async function processRow(row: NormalizedRow, settings: ProcessingSettings, priceMap: Map<string, number>): Promise<ProcessedRow> {
