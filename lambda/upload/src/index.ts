...
import { JobStatusItem, JobStatus, ProcessingSettings } from './types';  // Fixed: local import
...
async function generatePresignedUrl(fileKey: string, fileSize: number): Promise<string> {
  const command = new PutObjectCommand({
    Bucket: INPUT_BUCKET,
    Key: fileKey,
    ContentType: 'text/csv', // NOTE: ContentLength + ServerSideEncryption omitted intentionally.
    // Including them signs those values as S3 conditions — browsers cannot send
    // x-amz-server-side-encryption, which causes a 403. Enforce SSE at bucket level.
    Metadata: {
